...
<body class="antialiased text-gray-900 bg-gray-50 flex flex-col min-h-screen">
    ...
    <header class="bg-white shadow-sm sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex items-center justify-between">
            ...
            <nav class="hidden md:flex space-x-8">
                ...
                <a href="{{ route('portal.units') }}" class="text-sm font-bold text-[var(--gov-primary)] transition-colors border-b-2 border-[var(--gov-primary)] pb-1">Unidades</a>
            </nav>
                ...
                <ol class="inline-flex items-center space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-white font-medium">Unidades de Saúde</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-3xl md:text-4xl font-extrabold text-white font-sora tracking-tight">Rede de Atendimento</h1>
            <p class="mt-2 text-[var(--gov-secondary)] max-w-2xl">Consulte endereços, contatos e encontre o local de atendimento mais próximo de você.</p>
        </div>
    </div>
    ...
    <main class="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 -mt-10">
        ...
        <div class="flex justify-end mb-6 relative z-10">
            <div class="bg-white text-[var(--gov-primary-dark)] px-5 py-2.5 rounded-lg font-bold text-sm inline-flex items-center shadow-sm border border-gray-100">
                <svg class="w-5 h-5 mr-2 text-[var(--gov-accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
            ...
            <div class="bg-white border rounded-xl border-gray-100 shadow-md hover:shadow-xl transition-all hover:-translate-y-1 flex flex-col overflow-hidden group">
                <div class="p-1 min-h-[8px] bg-[var(--gov-primary)] w-full"></div>
                @if($unit->photo_path)
                    <img src="{{ Storage::url($unit->photo_path) }}" alt="Foto de {{ $unit->name }}" class="w-full h-48 object-cover border-b border-gray-100 group-hover:scale-105 transition-transform duration-500">
                @else
                    <div class="w-full h-48 bg-gray-100 flex items-center justify-center border-b border-gray-100">
                ...
                <div class="p-6 flex-1 bg-white relative z-10">
                    <div class="flex items-start justify-between">
                        <span class="bg-[var(--gov-secondary)] text-[var(--gov-primary-dark)] text-[10px] font-bold uppercase tracking-wider py-1 px-3 rounded-full border border-[var(--gov-primary)] opacity-80">
                            {{ $unit->kind ?? 'Unidade Básica' }}
                        </span>
                    </div>
                    <h4 class="text-xl font-bold font-sora text-gray-900 mt-4 mb-2 group-hover:text-[var(--gov-primary)] transition-colors">{{ $unit->name }}</h4>
                    @if($unit->description)
                    ...
                    <div class="mt-4 space-y-3">
                        <div class="flex items-start text-sm text-gray-600">
                            <svg class="w-5 h-5 text-[var(--gov-accent)] mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path></svg>
                            <span>{{ $unit->address ?: 'Endereço não informado' }}</span>
                        </div>
                        <div class="flex items-start text-sm text-gray-600">
                            <svg class="w-5 h-5 text-[var(--gov-accent)] mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                            <span>{{ $unit->phone ?: 'Telefone Indisponível' }}</span>
                        </div>
                        <div class="flex items-start text-sm text-gray-600">
                            <svg class="w-5 h-5 text-[var(--gov-accent)] mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            <span>Horário Oficial: 08:00 às 17:00</span>
                        </div>
                    </div>
                </div>
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 w-full text-center hover:bg-gray-100 transition-colors relative z-10">
                    <a href="{{ $unit->maps_link ?: 'https://maps.google.com/?q=' . urlencode($unit->address . ', Assaí - PR') }}" target="_blank" rel="noopener noreferrer" class="text-sm font-bold text-[var(--gov-primary)] hover:text-[var(--gov-primary-dark)] flex items-center justify-center w-full h-full">
                        Ver no Mapa 
            ...
            <div class="col-span-full py-16 text-center text-gray-500 bg-white rounded-xl shadow-sm border border-gray-200">
                ...
                <p class="text-lg font-medium text-gray-600">Nenhuma unidade de saúde cadastrada no momento.</p>
            </div>
            @endforelse
        </div>
    </main>
