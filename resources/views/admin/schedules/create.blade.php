    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Configure uma nova regra de atendimento e capacidade para a clínica.</p>
            </div>
        </div>
    </x-slot>

    <div class="max-w-4xl">
        <form action="{{ route('admin.schedules.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden p-6" x-data="{ clinic: '{{ old('clinic_type', \App\Models\WomenClinicAppointment::CLINIC_WOMEN) }}' }">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="specialty" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                        <!-- Women Clinic -->
                        <option value="{{ \App\Models\WomenClinicAppointment::SPECIALTY_CARDIOLOGIA }}" x-show="clinic === '{{ \App\Models\WomenClinicAppointment::CLINIC_WOMEN }}'" {{ old('specialty') == \App\Models\WomenClinicAppointment::SPECIALTY_CARDIOLOGIA ? 'selected' : '' }}>Cardiologia</option>
                        <option value="{{ \App\Models\WomenClinicAppointment::SPECIALTY_ORTOPEDIA }}" x-show="clinic === '{{ \App\Models\WomenClinicAppointment::CLINIC_WOMEN }}'" {{ old('specialty') == \App\Models\WomenClinicAppointment::SPECIALTY_ORTOPEDIA ? 'selected' : '' }}>Ortopedia</option>
                        <option value="{{ \App\Models\WomenClinicAppointment::SPECIALTY_PSIQUIATRIA }}" x-show="clinic === '{{ \App\Models\WomenClinicAppointment::CLINIC_WOMEN }}'" {{ old('specialty') == \App\Models\WomenClinicAppointment::SPECIALTY_PSIQUIATRIA ? 'selected' : '' }}>Psiquiatria</option>

                        <!-- Policlinica -->
                        <option value="{{ \App\Models\WomenClinicAppointment::SPECIALTY_ODONTOLOGIA }}" x-show="clinic === '{{ \App\Models\WomenClinicAppointment::CLINIC_POLICLINICA }}'" {{ old('specialty') == \App\Models\WomenClinicAppointment::SPECIALTY_ODONTOLOGIA ? 'selected' : '' }}>Odontologia</option>
                        <option value="{{ \App\Models\WomenClinicAppointment::SPECIALTY_FISIOTERAPIA }}" x-show="clinic === '{{ \App\Models\WomenClinicAppointment::CLINIC_POLICLINICA }}'" {{ old('specialty') == \App\Models\WomenClinicAppointment::SPECIALTY_FISIOTERAPIA ? 'selected' : '' }}>Fisioterapia</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="day_of_week" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                        ...
                        <option value="0" {{ old('day_of_week') == '0' ? 'selected' : '' }}>Domingo</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="time" name="time" value="{{ old('time', '08:00') }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="capacity" min="0" value="{{ old('capacity', 1) }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                        ...
                        @foreach([1, 2, 3, 4, 5] as $week)
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="weeks_of_month[]" value="{{ $week }}" 
                                       ...
                                       {{ in_array($week, old('weeks_of_month', [1, 2, 3, 4, 5])) ? 'checked' : '' }}>
